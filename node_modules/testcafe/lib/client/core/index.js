...
(function (modules) {
	...
	"testcafe-release": {
		"src": {
			...
			"client": {
				"browser": {
					"index.js": function (exports, module, require) {
						...
						var _browserConnectionTimeouts = require('../../utils/browser-connection-timeouts');

						function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

						var allowInitScriptExecution = false; // TODO: once we'll have client commons load it from there instead of node modules (currently it's leads to two copies of this packages on client)
						...
						function startHeartbeat(heartbeatUrl, createXHR) {
						    ...
						    window.setInterval(heartbeat, _browserConnectionTimeouts.HEARTBEAT_INTERVAL);
				...
				"core": {
					...
					"utils": {
						...
						"content-editable.js": function (exports, module, require) {
							...
							function getNearestCommonAncestor(node1, node2) {
							    if (domUtils.isTheSameNode(node1, node2)) {
							        ...
							        return _hammerhead.nativeMethods.nodeParentNodeGetter.call(node1);
							    }
							    ...
							    for (curNode = node1; curNode !== contentEditableParent; curNode = _hammerhead.nativeMethods.nodeParentNodeGetter.call(curNode)) {
							        ancestors.push(curNode);
							    }for (curNode = node2; curNode !== contentEditableParent; curNode = _hammerhead.nativeMethods.nodeParentNodeGetter.call(curNode)) {
							...
							function isNodeSelectable(node, includeDescendants) {
							    ...
							    var parent = _hammerhead.nativeMethods.nodeParentNodeGetter.call(node);
							    var isContentEditableRoot = !domUtils.isContentEditableElement(parent);
							...
							function hasWhiteSpacePreStyle(el, container) {
							    ...
							    while (el !== container) {
							        el = _hammerhead.nativeMethods.nodeParentNodeGetter.call(el);
						...
						"event.js": function (exports, module, require) {
							...
							exports.__esModule = true;
							exports.preventDefault = exports.WHICH_PARAMETER = exports.DOM_EVENTS = exports.BUTTONS_PARAMETER = exports.BUTTON = undefined;
							exports.bind = bind;
						...
						"get-time-limited-promise.js": function (exports, module, require) {
							...
							exports.default = function (promise, ms) {
							    return _hammerhead.Promise.race([promise, (0, _delay2.default)(ms).then(function () {
							        return _hammerhead.Promise.reject(new TimeLimitedPromiseTimeoutExpiredError());
							    })]);
							};
							...
							var _templates = require('../../../errors/runtime/templates');
							...
							var timeLimitedPromiseTimeoutExpiredTemplate = _templates2.default[_types.RUNTIME_ERRORS.timeLimitedPromiseTimeoutExpired];

							var TimeLimitedPromiseTimeoutExpiredError = function (_Error) {
							    _inherits(TimeLimitedPromiseTimeoutExpiredError, _Error);

							    function TimeLimitedPromiseTimeoutExpiredError() {
							        _classCallCheck(this, TimeLimitedPromiseTimeoutExpiredError);

							        var _this = _possibleConstructorReturn(this, _Error.call(this, timeLimitedPromiseTimeoutExpiredTemplate));

							        _this.code = _types.RUNTIME_ERRORS.timeLimitedPromiseTimeoutExpired;
							        return _this;
							    }
			...
			"errors": {
				"runtime": {
					"templates.js": function (exports, module, require) {
						...
						exports.default = (_RUNTIME_ERRORS$canno = {}, _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotCreateMultipleLiveModeRunners] = 'Cannot create multiple live mode runners.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotRunLiveModeRunnerMultipleTimes] = 'Cannot run a live mode runner multiple times.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.browserDisconnected] = 'The {userAgent} browser disconnected. This problem may appear when a browser hangs or is closed, or due to network issues.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotRunAgainstDisconnectedBrowsers] = 'The following browsers disconnected: {userAgents}. Tests will not be run.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotEstablishBrowserConnection] = 'Unable to establish one or more of the specified browser connections. This can be caused by network issues or remote device failure.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotFindBrowser] = 'Unable to find the browser. "{browser}" is not a browser alias or path to an executable file.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.browserProviderNotFound] = 'The specified "{providerName}" browser provider was not found.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.browserNotSet] = 'No browser selected to test against.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.testSourcesNotSet] = 'No test file specified.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.noTestsToRun] = 'No tests to run. Either the test files contain no tests or the filter function is too restrictive.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotFindReporterForAlias] = 'The provided "{name}" reporter does not exist. Check that you have specified the report format correctly.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.multipleStdoutReporters] = 'Multiple reporters attempting to write to stdout: "{reporters}". Only one reporter can write to stdout.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.optionValueIsNotValidRegExp] = 'The "{optionName}" option value is not a valid regular expression.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.optionValueIsNotValidKeyValue] = 'The "{optionName}" option value is not a valid key-value pair.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidSpeedValue] = 'Speed should be a number between 0.01 and 1.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidConcurrencyFactor] = 'The concurrency factor should be an integer greater or equal to 1.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotDivideRemotesCountByConcurrency] = 'The number of remote browsers should be divisible by the factor of concurrency.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.portsOptionRequiresTwoNumbers] = 'The "--ports" option requires two numbers to be specified.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.portIsNotFree] = 'The specified {portNum} port is already in use by another program.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidHostname] = 'The specified "{hostname}" hostname cannot be resolved to the current machine.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotFindSpecifiedTestSource] = 'Cannot find a test source file at "{path}".', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientFunctionCodeIsNotAFunction] = '{#instantiationCallsiteName} code is expected to be specified as a function, but {type} was passed.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.selectorInitializedWithWrongType] = '{#instantiationCallsiteName} is expected to be initialized with a function, CSS selector string, another Selector, node snapshot or a Promise returned by a Selector, but {type} was passed.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientFunctionCannotResolveTestRun] = "{#instantiationCallsiteName} cannot implicitly resolve the test run in context of which it should be executed. If you need to call {#instantiationCallsiteName} from the Node.js API callback, pass the test controller manually via {#instantiationCallsiteName}'s `.with({ boundTestRun: t })` method first. Note that you cannot execute {#instantiationCallsiteName} outside the test code.", _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.regeneratorInClientFunctionCode] = '{#instantiationCallsiteName} code, arguments or dependencies cannot contain generators or "async/await" syntax (use Promises instead).', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidClientFunctionTestRunBinding] = 'The "boundTestRun" option value is expected to be a test controller.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidValueType] = '{smthg} is expected to be a {type}, but it was {actual}.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.unsupportedUrlProtocol] = 'The specified "{url}" test page URL uses an unsupported {protocol}:// protocol. Only relative URLs or absolute URLs with http://, https:// and file:// protocols are supported.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.testControllerProxyCannotResolveTestRun] = 'Cannot implicitly resolve the test run in the context of which the test controller action should be executed. Use test function\'s \'t\' argument instead.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.timeLimitedPromiseTimeoutExpired] = 'Timeout expired for a time limited promise', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotUseScreenshotPathPatternWithoutBaseScreenshotPathSpecified] = 'Unable to set the screenshot path pattern when screenshots are disabled. Specify the base path where screenshots are stored to enable them.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotSetVideoOptionsWithoutBaseVideoPathSpecified] = 'Unable to set video or encoding options when video recording is disabled. Specify the base path where video files are stored to enable recording.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.multipleAPIMethodCallForbidden] = 'You cannot call the "{methodName}" method more than once. Pass an array of parameters to this method instead.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.invalidReporterOutput] = "Specify a file name or a writable stream as the reporter's output target.", _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotReadSSLCertFile] = 'Unable to read the "{path}" file, specified by the "{option}" ssl option. Error details:\n' + '\n' + '{err}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotPrepareTestsDueToError] = 'Cannot prepare tests due to an error.\n' + '\n' + '{errMessage}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotParseRawFile] = 'Cannot parse a test source file in the raw format at "{path}" due to an error.\n' + '\n' + '{errMessage}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.testedAppFailedWithError] = 'Tested app failed with an error:\n' + '\n' + '{errMessage}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.unableToOpenBrowser] = 'Was unable to open the browser "{alias}" due to error.\n' + '\n' + '{errMessage}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.requestHookConfigureAPIError] = 'There was an error while configuring the request hook:\n' + '\n' + '{requestHookName}: {errMsg}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.forbiddenCharatersInScreenshotPath] = 'There are forbidden characters in the "{screenshotPath}" {screenshotPathType}:\n' + ' {forbiddenCharsDescription}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotFindFFMPEG] = 'Unable to locate the FFmpeg executable required to record videos. Do one of the following:\n' + '\n' + '* add the FFmpeg installation directory to the PATH environment variable,\n' + '* specify the path to the FFmpeg executable in the FFMPEG_PATH environment variable or the ffmpegPath video option,\n' + '* install the @ffmpeg-installer/ffmpeg package from npm.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotFindTypescriptConfigurationFile] = 'Unable to find the TypeScript configuration file in "{filePath}"', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientScriptInitializerIsNotSpecified] = 'Specify the JavaScript file path, module name or script content to inject a client script.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientScriptBasePathIsNotSpecified] = 'Specify the base path for the client script file.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientScriptInitializerMultipleContentSources] = 'You cannot combine the file path, module name and script content when you specify a client script to inject.', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.cannotLoadClientScriptFromPath] = 'Cannot load a client script from {path}', _RUNTIME_ERRORS$canno[_types.RUNTIME_ERRORS.clientScriptModuleEntryPointPathCalculationError] = 'An error occurred when trying to locate the injected client script module:\n\n{errorMessage}.', _RUNTIME_ERRORS$canno);
						module.exports = exports['default'];
					}
				},
			...
			"utils": {
				"browser-connection-timeouts.js": function (exports, module, require) {
					...
					var HEARTBEAT_TIMEOUT = exports.HEARTBEAT_TIMEOUT = 2 * 60 * 1000;
					var BROWSER_RESTART_TIMEOUT = exports.BROWSER_RESTART_TIMEOUT = 60 * 1000;

					var HEARTBEAT_INTERVAL = exports.HEARTBEAT_INTERVAL = 2 * 1000;

					var CHECK_IFRAME_DRIVER_LINK_DELAY = exports.CHECK_IFRAME_DRIVER_LINK_DELAY = 500;
					var SEND_STATUS_REQUEST_TIME_LIMIT = exports.SEND_STATUS_REQUEST_TIME_LIMIT = 5000;
					var SEND_STATUS_REQUEST_RETRY_DELAY = exports.SEND_STATUS_REQUEST_RETRY_DELAY = 300;
					var SEND_STATUS_REQUEST_RETRY_COUNT = exports.SEND_STATUS_REQUEST_RETRY_COUNT = Math.floor(HEARTBEAT_TIMEOUT / SEND_STATUS_REQUEST_RETRY_DELAY - 1);
					var CHECK_STATUS_RETRY_DELAY = exports.CHECK_STATUS_RETRY_DELAY = 1000;
				}
			}
		}
	}
